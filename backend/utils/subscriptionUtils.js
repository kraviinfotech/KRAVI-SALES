...
const buildSubscriptionStatus = async (managerId) => {
  ...
  if (!subscription) {
    return {
      ...
      daysRemaining: 0,
      currentSellers,
      paymentSettingsReady: Boolean(settings?.paymentKeyId && settings?.paymentKeySecret)
    };
  }
...
const createRazorpayOrder = ({ keyId, keySecret, amount, currency, receipt, notes }) => new Promise((resolve, reject) => {
  ...
  const request = https.request({
    ...
    response.on('end', () => {
      ...
      return reject(new Error(parsed.error?.description || 'Razorpay order creation failed'));
    });
  });
