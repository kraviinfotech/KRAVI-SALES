  ...
  <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
    <div className="flex items-center gap-2">
      <div className="relative flex-1">
        ...
        <input id="manager-search" value={value} onChange={(e) => onChange(e.target.value)} placeholder="Search by name, email, or phone." aria-label="Search managers" className="w-full rounded-xl border border-gray-100 bg-gray-50 py-2 pl-10 pr-4 outline-none focus:ring-2 focus:ring-[#6C3EF4]" />
      </div>
      <button type="button" aria-label="Filter managers" className="rounded-xl border border-gray-100 bg-gray-50 p-2 text-gray-600"><Filter size={18} /></button>
    </div>
  </div>
);
  ...
  <div className="fixed inset-0 z-50 flex justify-end bg-black/50">
    <div className="h-full w-full max-w-md overflow-y-auto bg-white shadow-2xl">
      <div className="sticky top-0 flex justify-between bg-gradient-to-r from-[#6C3EF4] to-purple-600 p-6 text-white">
        ...
        <button type="button" onClick={onClose} aria-label="Close manager profile"><X size={24}/></button>
      </div>
